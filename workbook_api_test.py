...
# I don't want to spend the time to figure
# which of these fields are always present
COMPANY_INFO_KEYS = set({
  'Id',
  'Name',
  'Initials',
...
def test_get_company():
  ...
  for c in companies:
    assert COMPANY_KEYS.issubset(c.keys()), "Company entry has all keys"
    company = api.get_company(CompanyId=c['Id'])
    print(COMPANY_INFO_KEYS - company.keys())
    assert COMPANY_INFO_KEYS.issubset(company.keys()), "Company has all keys"
